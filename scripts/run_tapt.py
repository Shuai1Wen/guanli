...
Task-Adaptive Pre-Training (TAPT) 实现，符合CLAUDE.md要求：
- 基础模型：results/dapt_model（DAPT输出的政策领域适配模型）
...
class TAPTConfig:
    ...
    # 模型配置
    base_model: str = "results/dapt_model"  # DAPT输出的模型
    tokenizer_max_length: int = 512

    # 数据配置
    corpus_path: str = "data/tapt_corpus.txt"
    train_test_split: float = 0.90  # 90%训练，10%验证（语料较小）

    # MLM配置
    mlm_probability: float = 0.15

    # 训练超参数（调整以适应小数据集和避免遗忘）
    num_train_epochs: int = 5  # 更多轮数
    per_device_train_batch_size: int = 8  # 更小batch size
    per_device_eval_batch_size: int = 16
    learning_rate: float = 2e-5  # 更小学习率（避免灾难性遗忘）
    weight_decay: float = 0.01
    warmup_steps: int = 200  # 更少warmup
    gradient_accumulation_steps: int = 2  # 有效batch size = 8*2 = 16

    # 输出配置
    output_dir: str = "results/tapt_checkpoints"
    final_model_dir: str = "results/tapt_model"
...
class TAPTTrainer:
    ...
    def __init__(self, config: Optional[TAPTConfig] = None):
        """初始化训练器

        Args:
            config: 训练配置对象
        """
        self.config = config if config is not None else TAPTConfig()
        ...
        if not Path(self.config.base_model).exists():
            raise FileNotFoundError(
                f"DAPT模型不存在: {self.config.base_model}\n"
                f"请先运行 run_dapt.py 进行DAPT训练"
            )
    ...
    def load_and_preprocess_dataset(self):
        ...
        # 检查数据集大小
        if len(dataset) < 100:
            print(f"\n  ⚠️  警告：数据集太小（{len(dataset)}样本）")
            print(f"  建议：")
            print(f"    1. 增加标注数据量")
            print(f"    2. 检查prep_tapt_task.py的参数设置")
    ...
    def train(self):
        ...
        print(f"\n下一步：")
        print(f"  1. 使用TAPT模型更新RAG检索系统（修改retrieve_evidence.py）")
        print(f"  2. 重新构建FAISS索引（run build_index.py）")
        print(f"  3. 评测F1性能是否达到≥0.85")
