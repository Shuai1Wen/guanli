...
## 省级数据采集分层战略方案深度分析（2025-11-23）

### 任务背景
用户要求：
1. 使用sequential-thinking深度分析31省份数据采集战略
2. 将31个省份分成A/B/C/D四类
3. 为每一类制定具体的处理策略
4. 设计可执行的分层实施方案
5. 核心战略思想："不是31省全覆盖，而是按风险类型分层，有控制地只解决最重要的一半"

### 分析方法
1. 检索现有配置文件（seeds_sites.yaml）
2. 分析31个省份的完整列表（province_codes.csv）
3. 评估已验证省份（11个）的大区覆盖情况
4. 识别关键空白（东北0覆盖、西部覆盖不足）
5. 基于DID样本量需求和图学习多样性需求，制定战略方案
6. 对比三种路径（保守/平衡/激进），选择最优方案

### 核心发现

#### 发现1：现有11个已验证省份的大区覆盖不均
**统计**：
- 华南：1/3 (33%)
- 华东：5/6 (83%) ⚠️ 但浙江无法访问、江苏用动态接口
- 华北：1/5 (20%)
- 华中：2/3 (67%)
- 西南：2/5 (40%)
- 西北：1/5 (20%)
...
**致命问题**：东北工业基地、国企重镇，政策逻辑与华东华南差异大，0覆盖会被审稿人质疑"样本代表性"

#### 发现2：浙江省和江苏省存在技术风险
**浙江省**：
- 配置注释："verified: false  # 暂时无法访问，稍后重试"
- 实验室网络环境受限
- 建议：降级为D类

**江苏省**：
- 使用`dataproxy.jsp`动态接口
- 稳定性存疑，可能随时变更
- 建议：尝试C类策略（专用解析器），失败后降级

**影响**：11个已验证 → 实际可能只有9个稳定（排除浙江、江苏）
...
**处理**：保留详细配置（行1-277），删除简化配置（行278-297）
...
**B4子类：其他地区（4个）- 低优先级**
- 天津市（12）、河北省（13）、山西省（14）、湖南省（43）
- 华北和华中已有覆盖，暂不验证

**战略决策**：
- ✅ 优先验证5个：辽宁、黑龙江、贵州、甘肃、福建
- ✅ 快速验证流程（每省30分钟）：robots.txt → 抓取首页 → 抓取1个详情页
- ✅ 成功标准：5个中至少成功3个

#### C类：特殊分页或动态接口（3个）
1. **江苏省**（32）- `dataproxy.jsp`动态接口，2次尝试上限
2. 内蒙古自治区（15）- 待验证后决定
3. 广西壮族自治区（45）- 待验证后决定

**策略**：
- 🔧 江苏省：DevTools分析dataproxy.jsp参数，编写专用解析器
- 🔧 内蒙古、广西：先按B类验证，失败后降级C类
- ❌ 2次尝试均失败 → 降级为D类

#### D类：访问困难或禁止（5个）
1. **浙江省**（33）- 实验室网络无法访问
2. 西藏自治区（54）- 推测网络环境受限
3. 新疆维吾尔自治区（65）- 推测网络环境受限
4. 海南省（46）- 待验证（可能403）
5. （其他验证失败的）
...
**核心理由**：
1. **填补关键空白**：东北0覆盖是致命伤，路径B补充辽宁、黑龙江
2. **成本-效益最优**：5个B类验证预计10-15小时，边际收益高
3. **DID样本量充分**：13-15个省份满足CS-ATT文献推荐（10-20个单元）
4. **图学习多样性**：7大区全覆盖 + 发达-欠发达 + 工业-科技中心
5. **风险可控**：预期成功3-4个，即使仅成功3个仍有12个样本
...
**任务1.1：清理配置文件重复项**（1小时）
- 删除seeds_sites.yaml行278-297的简化配置
- 保留行1-277的详细配置
- 验证YAML语法正确性

**任务1.2：测试不稳定站点**（30分钟）
- 浙江省：尝试4G热点访问，失败则确认降级D类
- 江苏省：测试dataproxy.jsp当前状态
- 更新`verified`字段

**任务1.3：A类三点抽样校验**（3小时）
- 验证9个A类省份的稳定性
- 每省抓取3个时间点文档（2024、2020、2015年各1篇）
- 成功标准：9个中至少8个通过

#### 阶段2：B类优先验证（5-7天）

**任务2.1：快速验证5个B类省份**（2.5小时）
- 辽宁省、黑龙江省、贵州省、甘肃省、福建省
- 每省30分钟：robots.txt → 首页 → 详情页
- 成功标准：链接数>10，内容>100字

**任务2.2：测试模式爬取（3页）**（5小时）
```bash
python scripts/crawl_provinces.py 辽宁省 --test
...
- DevTools分析POST参数（page, appid, webid, columnid, unitid）
- 编写专用解析器
- 2次尝试上限，失败则降级D类
...
- A类（广东、上海、北京）：max_pages=20
- 其他A类：max_pages=10
- B类成功省份：max_pages=10

**预计文档量**：
- 9个A类 × 50条/省 × 10-20页 = 4,500-9,000条
- 5个B类 × 30条/省 × 10页 = 1,500条
...
**补偿方案**：
1. 子样本分析："全样本（13省）"vs"去除东北（11省）"对比
2. 外部语料补充：ChpoBERT吉林省政策文本
3. 透明度报告：论文方法论部分说明缺失原因

#### 局限性2：西部地区样本不足（5/10 = 50%）
**补偿方案**：
1. 代表性论证：贵州、甘肃作为西部欠发达地区代表
2. 加权分析：使用GDP/R&D占比加权
3. 安慰剂检验：缺失省份统计指标作为安慰剂处理

#### 局限性3：D类省份的非随机缺失
**补偿方案**：
1. 缺失机制分析：倾向得分匹配（PSM）检验缺失是否随机
2. Heckman两阶段：控制逆米尔斯比率（IMR）
3. 灵敏度分析：报告"最坏情况"和"最好情况"

#### 局限性4：图学习的节点稀疏性
**补偿方案**：
1. 虚拟节点：D类省份使用外部语料特征，边权重降权（0.3-0.5）
2. 降低地理权重：提高政策语义相似度边权重
3. 子图学习：7个大区子图，再融合
...
**理由**:
- 填补东北空白（0→2个）
- 成本-效益最优（2-3周获得13-15个样本）
- DID样本量充分（满足CS-ATT文献推荐）
- 图学习多样性（7大区全覆盖）
- 风险可控（预期成功率80%，有兜底方案）
...
**理由**:
- 浙江省：实验室网络无法访问，降级D类
- 江苏省：dataproxy.jsp稳定性存疑，降级C类（尝试）或D类（失败后）
- A类稳定性优先于数量
...
**时间**: 2025-11-23
**理由**:
- 边际收益递减：13-15个样本已满足统计效力
- 维护成本递增：每增加1个省份+10-15小时/年
- 质量优于数量：13个高质量 > 25个低质量
- 研究设计灵活性：13个足够支撑多种稳健性检验

### 输出文件

**战略分析报告**：
- `.claude/strategic-analysis-province-classification.md`（完整版，约1200行）
  - 执行摘要（核心结论、战略评分、关键决策）
  - 31省份完整分类（A/B/C/D四类）
  - 战略路径选择与论证（路径A/B/C对比）
  - 分层实施路线图（4个阶段详细任务）
  - 研究设计的局限性处理（4个局限性+补偿方案）
  - 关键成功要素（4个要素）
  - 可执行的Action List（立即/短期/中期/长期）
  - 风险缓解矩阵（8个风险+缓解措施）
  - 战略评分详解（4个维度详细评分）
  - 附录（关键决策依据、备选B类清单、DID样本量文献）

**上下文摘要**（已更新）：
- `.claude/context-summary-province-strategy.md`（简化版，约80行）
...
**长期（研究设计）**：
- DID样本量充分（13-15省 × 5-10年 = 65-150个观测值）
- 图学习多样性（7大区 + 发达-欠发达 + 工业-科技中心）
- 外部效度可论证（经济占比 + 稳健性检验 + 子样本分析）
...
1. 是否认可路径B（平衡策略）？
2. 是否同意剔除浙江省和江苏省为A类？
3. 是否同意优先验证5个B类（辽宁、黑龙江、贵州、甘肃、福建）？

**用户确认后**：
- 立即执行Week 1的Action List（任务1.1-1.4）
- 预计2-3周完成13-15个样本省份数据采集

### 总结

**核心战略**：按风险类型分层，有控制地解决最重要的一半
